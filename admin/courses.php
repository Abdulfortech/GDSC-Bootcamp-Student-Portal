...
<head>
	...
	<!-- title -->
	<title>Mobilizing App - Courses</title>
	<!-- Favicon -->
	...
	<section class="farko p-3 mt-5">
        ...
        <div class="col-md-10">    
		    ...
		    <div class="container text-center mt-4 page-card">
                <h1 class="featurette-heading fw-bold mt-4 mb-5 text-success">Register <span class="text-color2">a Course</span></h1>
                <form method="POST" action="process.php">
                    ...
                    <button class="btn btn-md mt-3 btn-success" type="submit" name="addCourse">Add Course</button>
                </form>


                <h1 class="featurette-heading fw-bold mt-4 mb-5 text-success">All <span class="text-color2">Courses</span></h1>
                ...
                <table id="example1" class="table table-bordered table-striped text-start">
                    ...
                    <tbody>
                        <?php
                            $courses = $conn->query("SELECT * FROM courses WHERE status=1 ") or die($conn->error);
                            $count = 1;
                        ...
                        <tr>
                            ...
                            <td><?= $course['code']?></td>
                            <td>
                                <?php if($course['status'] == 1){echo '<span class="badge bg-success badge-pill">Active</span>';}
                                    else{echo '<span class="badge bg-warning badge-pill">Inactive</span>';}?>
                            </td>
                            <td>
                                <a href="course?id=<?= $course['course_id']?>" class="btn btn-sm btn-success shadow-sm">
                                    <i class="fa fa-edit fa-sm text-white"></i>
                                </a>
                                <a href="process.php?action=DeleteCourse&id=<?= $course['course_id']?>" class="btn btn-sm btn-danger shadow-sm">
