  ...
  <main>
    ...
    <?php include 'navbar.php';
        if($_GET){
          ...
          $courseINFOR = $conn->query("SELECT * FROM courses WHERE course_id='".$courseid."' ") or die($conn->error);
          while ($row = $courseINFOR->fetch_assoc()) {
            $title = $row['title'];
            $code = $row['code'];
            $status = $row['status'];
          }
	...
	<section class="farko p-3 mt-5">
        ...
        <div class="col-md-5">    
            ...
            <h1 class="featurette-heading fw-bold mt-4 mb-5 text-success">Edit <span class="text-color2">Course</span></h1>
            <form method="POST" action="process.php">
                <input type="text" value="<?= $courseid?>" name="id" hidden>
                ...
                <button class="w-100 btn btn-lg btn-success" type="submit" name="editCourse">Save Course</button>
            </form>
            </div>
        </div>
        </center>
