...
<head>
	...
	<!-- title -->
	<title>Mobilizing App - Departments</title>
	<!-- Favicon -->
        ...
        <div class="col-md-10">    
		    ...
		    <div class="container text-center mt-4 page-card">
                <h1 class="featurette-heading fw-bold mt-4 mb-5 text-success">Add 
                    <span class="text-color2">Department</span>
                </h1>
                <form method="POST" action="process.php">
                    ...
                    <button class="btn btn-md mt-3 btn-success" type="submit" name="addDepartment">Add Department</button>
                </form>

                <h1 class="featurette-heading fw-bold mt-4 mb-5 text-success">All 
                    <span class="text-color2">Departments</span>
                </h1>                
                <table id="example1" class="table table-bordered table-striped text-start">
                    ...
                    <tbody>
                        <?php
                            $departments = $conn->query("SELECT * FROM departments WHERE status=1 ORDER BY department_id DESC") or die($conn->error);
                            $count = 1;
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if($department['status'] == 1){echo '<span class="badge bg-success badge-pill">Active</span>';}
                                    else{echo '<span class="badge bg-warning badge-pill">Inactive</span>';}?>
                            </td>
                            <td>
                                <a href="department?id=<?= $department['department_id']?>" class="btn btn-sm btn-success shadow-sm">
                                    <i class="fa fa-edit fa-sm text-white"></i>
                                </a>
                                <a href="process.php?action=DeleteDepartment&id=<?= $department['department_id']?>" class="btn btn-sm btn-danger shadow-sm">
